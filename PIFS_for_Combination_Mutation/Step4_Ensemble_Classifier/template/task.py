...
id_t, seqs_embd_cls_t, seqs_embd_class_t, seqs = Emb.get_embed_dataset_test()
...
for tol_ in tol_list:
    for C_ in C_list:
        for max_iter_ in max_iter_list:
            params_dic[param_num] = [tol_,C_,max_iter_]
            param_num += 1
...
params = params_dic[index_]
pred_num = 0
for val_keys_x, train_keys_x, val_keys_y, train_keys_y in data_fold:
    model = LogisticRegression(penalty='l2', dual=True, tol=params[0], C=params[1], max_iter=params[2],
                               fit_intercept=True, intercept_scaling=1, class_weight='balanced',
                               solver='liblinear', n_jobs=1)
    model.fit(input_x12(train_keys_x), train_keys_y)
    test_y_pred = model._predict_proba_lr(input_x12(seqs_embd_cls_t))
    test_y_pred_ = np.argmax(test_y_pred, axis=1)
    test_y_pred_auc = test_y_pred[:, 1]
    data = {'mut_id': id_t,
            'pred_proba': test_y_pred_auc,
            'pred': test_y_pred_}
    df = pd.DataFrame(data)
    df.to_csv('pred' + str(pred_num) + '.csv', index=False)
    pred_num += 1